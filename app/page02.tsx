...
// Define the sections with their background colors
const sections = [
  {
    id: "photography",
    label: "Photography",
...
export default function Home() {
  ...
  // Get the current section's background color
  const currentBgColor = sections.find((section) => section.id === activeSection)?.bgColor || "bg-black"

  // Determine text colors based on background
  const isWhiteBackground = currentBgColor === "bg-white"

  return (
    <main className={`relative min-h-screen flex flex-col transition-colors duration-500 ${currentBgColor}`}>
